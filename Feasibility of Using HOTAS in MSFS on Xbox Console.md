...
## 2024-01-29 Made Bluetooth Work for Old Mac

Despite that I developed this project on a Mac with Apple M2 chip, eventually I’d like to run it on my 2013 model MacBook Pro that has long been unused, so that I don’t have to tie my development machine with an Xbox console. It is definitely a low-end machine according to the standard today, and it cannot install MacOS of versions later than Big Sur, but it should be competent for just running the output program.

With simple tweaks, I was able to make Xcode output a runnable program for that machine. However, I found that when it used Bluetooth to transmit data, it was very slow especially when I used the throttle (which generated lots of keyboard events without rate limiting). It may still be sending data for 2~3 seconds after I stopped moving the throttle. Besides, some events are likely out of order since the throttle appeared to jump back to a previous location in the game.

I first tried to switch from the [writeSync](https://developer.apple.com/documentation/iobluetooth/iobluetoothrfcommchannel/1434633-writesync?language=objc) API to [writeAsync](https://developer.apple.com/documentation/iobluetooth/iobluetoothrfcommchannel/1430167-writeasync?language=objc), hoping to reduce the burden of the sending thread. That may have helped a bit, but this new API keeps returning a mysterious error code: -536870181. The search engine says other folks have only encountered this when they used Final Cut Pro to render videos, which obviously doesn’t apply to my case. I also tried to override the delegate method [rfcommChannelWriteComplete](https://developer.apple.com/documentation/iobluetooth/iobluetoothrfcommchanneldelegate/1434978-rfcommchannelwritecomplete?language=objc) which should be invoked after the data is transmitted, however, its return status read no error. Also note that if I was just pressing buttons on the throttle, I wouldn’t get this error code.

My first guess was that some system framework detects race conditions in buffer access. This is an async interface, but I didn’t really wait until the previously chunk of data getting fully transmitted before overwriting it with new data. Is the system trying to warn me about that? I tried to create a large enough circular buffer so no data overwriting is involved, but I was still getting the same error.

My second guess was that this old Mac cannot transmit that much data over Bluetooth in a short period of time. I was not suspecting this to be a receiver side problem because it didn’t occur when I used my development machine. So, I added rate limiting on the data sender side. Note that the throttle is mapped to 11 key codes. If the throttle just moves a tiny little bit, the output would probably still be mapped to the same key code, but dozens of events may have been emitted. By reducing the number of repeating events, I finally got rid of this error code, so this is the root cause.

This behavior can be customized by setting [rate_limiting_threshold_ms](https://github.com/lun0522/HotasRemapper/blob/6a2b56a31f1e8fa79909e410a77989ebc5a5b8c5/HotasRemapperLib/src/protos/settings.proto#L13). I set it to 16 milliseconds, meaning if the throttle is just repeatedly outputting values that are mapped to the same key code, we only send an event over Bluetooth every 16ms, while a event with a different key code will be sent immediately if they come up. I chose this number because my monitor refreshes at 60Hz so a new frame is rendered every 16.67ms. There might be better ways to determine this number.
